...
CmdTrail currently has one repository owner and one release-capable maintainer,
`@yhay81`. This is an explicit continuity risk, not evidence of second-person
...
- the released native binary by running `cmdtrail --version`.
...
GH_TOKEN="$(gh auth token)" \
  ./scripts/continuity-drill.sh yhay81/cmdtrail cmdtrail
```
