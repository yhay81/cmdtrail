...
CmdTrail publishes native archives for Linux x86_64, macOS Apple Silicon and
...
archive="cmdtrail-${version}-${platform}.tar.gz"
gh release download "$version" --repo yhay81/cmdtrail \
...
install -m 0755 "${archive%.tar.gz}/cmdtrail" "$HOME/.local/bin/cmdtrail"
"$HOME/.local/bin/cmdtrail" --version
```
...
```powershell
$version = "v0.3.0"
$archive = "cmdtrail-$version-windows-x86_64.zip"
gh release download $version --repo yhay81/cmdtrail `
  --pattern $archive --pattern "SHA256SUMS"
...
Copy-Item "cmdtrail-$version-windows-x86_64\cmdtrail.exe" `
  (Join-Path $bin "cmdtrail.exe") -Force
& (Join-Path $bin "cmdtrail.exe") --version
```
...
`$HOME/.local/bin/cmdtrail` on macOS/Linux or
`$HOME\.local\bin\cmdtrail.exe` on Windows. Receipts and observed files are not
